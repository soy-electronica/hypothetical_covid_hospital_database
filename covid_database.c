...
//**  The implementation for the following code was a final project for an electrical & computer engineering undergraduate class in intermediate programming at the Univeristy of Maryland College Park. The project due date was three weeks from the date of assignment. The project involves a hypothetical hospital and patient database. The design requirements include managing patients' information. The project helped students master basic build, search, insert, and delete linked list operations in addition to basic graph algorithms, pointer concepts, and dynamic memory allocation **/
